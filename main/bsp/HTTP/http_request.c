...
/* 如果定义了USE_EXTERNAL_WIFI，则WiFi由外部管理（main项目）*/
#define USE_EXTERNAL_WIFI 1

#if !USE_EXTERNAL_WIFI
// Wi-Fi 配置 - 请根据实际网络修改
...
// 服务器配置
#define SERVER_URL "http://118.195.133.25:6060/api/health/upload"

#define ALARM_DEFAULT_HOST "118.195.133.25"
...
static void log_alarm_snapshot(const alarm_list_t *list) {
  ...
  for (size_t i = 0; i < list->count; ++i) {
    ...
    ESP_LOGI(TAG,
             "id=%d type=%d status=%d time=%s date=%s repeat=%s "
             "next=%04d-%02d-%02d %02d:%02d:%02d",
             a->id, a->type, a->status, a->alarm_time, a->target_date,
             a->repeat_days, (a->next_trigger > 0) ? (nt.tm_year + 1900) : 0,
...
void wifi_init_sta(void) {
  /* WiFi由外部管理，这里不做任何事 */
  ESP_LOGI(TAG, "WiFi managed externally, skipping init");
}
#else
/* 独立WiFi管理 */
...
static bool parse_time_of_day(const char *time_str, int *hour, int *minute,
                              int *second) {
...
static bool parse_date_ymd(const char *date_str, int *year, int *month,
                           int *day) {
...
static bool build_datetime(const char *date_str, const char *time_str,
                           struct tm *out) {
...
static uint8_t parse_repeat_mask(const cJSON *node, char *repeat_buf,
                                 size_t buf_len) {
    ...
    cJSON_ArrayForEach(child, node) {
      ...
      if (idx >= 0) {
        ...
        if (repeat_buf && buf_len > 0 && written < buf_len - 1) {
          int n = snprintf(repeat_buf + written, buf_len - written,
                           (written > 0) ? ",%d" : "%d", val);
          if (n > 0) {
            written += (size_t)n;
          }
...
esp_err_t http_send_health_data(const health_data_t *data) {
  ...
  cJSON_AddStringToObject(root, "sleepStatus",
                          data->sleep_status[0] ? data->sleep_status
                                                : "UNKNOWN");
  cJSON_AddNumberToObject(root, "motionIndex", data->motion_index);
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGE(TAG,
             "Please check if Server IP is correct and Port 6060 is open.");
  }
...
esp_err_t http_set_alarm_server(const char *host, uint16_t port) {
  if (!host || strlen(host) == 0 || strlen(host) >= sizeof(s_alarm_host) ||
      port == 0) {
    return ESP_ERR_INVALID_ARG;
  }
...
esp_err_t http_set_alarm_user(const char *user_id) {
  if (!user_id || strlen(user_id) == 0 ||
      strlen(user_id) >= sizeof(s_alarm_user)) {
    return ESP_ERR_INVALID_ARG;
  }
...
time_t alarm_compute_next_trigger(const alarm_info_t *alarm,
  ...
  if (alarm->type == ALARM_TYPE_ONCE) {
    ...
    if (target_min_ts == (time_t)-1 || now_min_ts == (time_t)-1 ||
        target_min_ts < now_min_ts) {
      return 0;
    }
    return target_ts;
  }
...
esp_err_t http_fetch_alarms(alarm_list_t *out_list) {
  ...
  char url[128];
  snprintf(url, sizeof(url), "http://%s:%u/api/alarms/list/%s", s_alarm_host,
           (unsigned)s_alarm_port, s_alarm_user);

  http_resp_buffer_t resp = {
      .cap = 2048, .len = 0, .data = (char *)calloc(1, 2048)};
  ...
  cJSON_ArrayForEach(item, alarms) {
    ...
    dst->type = cJSON_IsNumber(type) ? (alarm_type_t)((int)type->valuedouble)
                                     : ALARM_TYPE_ONCE;
    dst->status = cJSON_IsNumber(status) ? (int)status->valuedouble : 1;

    if (cJSON_IsString(alarm_time) && alarm_time->valuestring) {
      snprintf(dst->alarm_time, sizeof(dst->alarm_time), "%s",
               alarm_time->valuestring);
    }

    if (cJSON_IsString(target_date) && target_date->valuestring) {
      snprintf(dst->target_date, sizeof(dst->target_date), "%s",
               target_date->valuestring);
    }

    dst->repeat_mask = parse_repeat_mask(repeat_days, dst->repeat_days,
                                         sizeof(dst->repeat_days));
...
esp_err_t http_update_alarm_status(int alarm_id, int status) {
  ...
  char url[160];
  snprintf(
      url, sizeof(url), "http://%s:%u/api/alarms/%d/status?userId=%s&status=%d",
...
static void alarm_fetch_task_fn(void *arg) {
  ...
  while (1) {
    if (http_fetch_alarms(latest) == ESP_OK) {
      ...
      if (s_alarm_mutex &&
          xSemaphoreTake(s_alarm_mutex, pdMS_TO_TICKS(2000)) == pdTRUE) {
        s_alarm_list = *latest;
...
static void alarm_monitor_task_fn(void *arg) {
  while (1) {
    ...
    if (s_alarm_mutex &&
        xSemaphoreTake(s_alarm_mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
        ...
        if (alarm_is_due(alarm, &now_tm)) {
          if (s_alarm_cb) {
            ...
            ESP_LOGI(TAG, "Alarm %d due at %s %s", alarm->id,
                     (alarm->target_date[0] != '\0') ? alarm->target_date
                                                     : "repeat",
                     alarm->alarm_time);
          }
...
esp_err_t alarm_service_start(uint32_t fetch_interval_ms, alarm_trigger_cb_t cb,
                              void *cb_ctx) {
  ...
  if (!s_alarm_fetch_task) {
    BaseType_t r =
        xTaskCreate(alarm_fetch_task_fn, "alarm_fetch", ALARM_TASK_STACK, NULL,
                    ALARM_TASK_PRIO, &s_alarm_fetch_task);
    if (r != pdPASS) {
  ...
  if (!s_alarm_monitor_task) {
    BaseType_t r =
        xTaskCreate(alarm_monitor_task_fn, "alarm_monitor", ALARM_TASK_STACK,
                    NULL, ALARM_TASK_PRIO, &s_alarm_monitor_task);
    if (r != pdPASS) {
