  ...
  while (1) {
    ...
    if (xSemaphoreTake(s_alarm_music_sem, portMAX_DELAY) == pdTRUE) {
      ...
      for (int retry = 0; retry < 3 && play_ret != ESP_OK; retry++) {
        if (retry > 0) {
          ESP_LOGW(TAG, "音乐播放启动重试 (%d/3)...", retry + 1);
          vTaskDelay(pdMS_TO_TICKS(1000));
        }
        play_ret = audio_player_start();
      }

      if (play_ret != ESP_OK) {
        ESP_LOGE(TAG, "启动音乐播放失败（SD卡可能未插入）");
        continue;
      }
      ...
      while (!s_alarm_music_stop) {
        ...
        if (stage == SLEEP_STAGE_WAKE) {
          if (wake_seen_tick == 0) {
            wake_seen_tick = now;
          } else if ((now - wake_seen_tick) * portTICK_PERIOD_MS >=
                     wake_stop_delay_ms) {
            ESP_LOGI(TAG, "检测到清醒超过%u ms，闹钟停止",
                     (unsigned)wake_stop_delay_ms);
        ...
        if (stage == SLEEP_STAGE_REM) {
          ...
          if ((now - last_deep_increase_tick) * portTICK_PERIOD_MS >=
              deep_increase_period_ms) {
      ...
      if (stopped_by_key2) {
        ...
        ESP_LOGI(TAG, "KEY2已停止，抑制重复响铃 %u ms",
...
esp_err_t alarm_music_start(void) {
  ...
  BaseType_t res = xTaskCreate(alarm_music_task_fn, "alarm_music", 4096, NULL,
                               5, &s_alarm_music_task);
  if (res != pdPASS) {
