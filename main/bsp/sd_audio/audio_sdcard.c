...
esp_err_t audio_sdcard_mount(void) {
  ...
  if (!s_bus_initialized) {
    esp_err_t bus_ret =
        spi_bus_initialize(host.slot, &s_bus_cfg, SDSPI_DEFAULT_DMA);
  ...
  for (int retry = 0; retry < max_retries; retry++) {
    if (retry > 0) {
      ESP_LOGW(TAG, "SD卡挂载重试 (%d/%d)...", retry + 1, max_retries);
      vTaskDelay(pdMS_TO_TICKS(500));
    }

    mount_ret = esp_vfs_fat_sdspi_mount(AUDIO_SD_MOUNT_POINT, &host,
  ...
  if (mount_ret != ESP_OK) {
    ESP_LOGE(TAG, "mount sd card failed: %s (请检查SD卡是否插入)",
...
bool audio_sdcard_is_mounted(void) { return s_mounted; }
